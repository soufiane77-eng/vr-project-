...
const getFramePath = (index) => {
  return `https://cdn.jsdelivr.net/gh/soufiane77-eng/vr-project-@master/public/frames/frame_${index.toString().padStart(5, '0')}.webp`;
};
...
// ============================================
// 3D Logo — Three.js + GLB loader
// ============================================
import * as THREE from 'three';
import { GLTFLoader } from 'three/addons/loaders/GLTFLoader.js';

function init3DLogo() {
  const canvasEl = document.getElementById('logo-3d-canvas');
  if (!canvasEl) return;
  ...
  const loader = new GLTFLoader();
    ...
    // Apply 50% orange / 50% white gradient material
    const orangeColor = new THREE.Color(1.0, 0.4, 0.0); // orange
    const whiteColor = new THREE.Color(1.0, 1.0, 1.0);  // white
    const gradientMaterial = new THREE.ShaderMaterial({
      uniforms: {
        uOrange: { value: orangeColor },
        uWhite: { value: whiteColor },
        uMinY: { value: ext.y * -0.5 * scale },
        uMaxY: { value: ext.y * 0.5 * scale },
      },
      vertexShader: `
        varying float vY;
        void main() {
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          vec3 color = mix(uOrange, uWhite, t);
